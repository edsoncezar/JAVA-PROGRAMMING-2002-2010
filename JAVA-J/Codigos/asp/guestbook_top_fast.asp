...
strFile = Server.MapPath("guestbook_top_fast.txt")
...
If Request.Form("name") = "" Or Request.Form("comment") = "" Then
	...
	<form action="guestbook_top_fast.asp" method="post">
	<table>
		<tr>
	...
	<!-- #include file="guestbook_top_fast.txt" -->
	<%
Else
	...
	' Open the TextFile and read the entire contents into memory
	Set objFile = objFSO.OpenTextFile(strFile, 1, False)
	strBuffer = objFile.ReadAll
	objFile.Close

	' Overwrite the old file with a blank one and dump new entry
	' into it followed by all the previous.
	...
	' Return the previous results to the file
	...
	<a href="guestbook_top_fast.asp">Back to the guestbook</a>
	<%
End If
